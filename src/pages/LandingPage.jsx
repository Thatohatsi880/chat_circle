      ...
      <nav className="justify-between m-0 mt-4">
        <div className="container mx-auto flex justify-between items-center flex-col md:flex-row gap-6">
          <div className="flex justify-between items-center w-full md:w-auto">
            <button onClick={toggleMenu} className="text-gray-600 focus:outline-none md:hidden">
              {isMenuOpen ? <FiX size={24} /> : <FiMenu size={24} />}
            </button>
            <Link to="/" className="text-white text-xl font-bold"><img src={logo} alt="chat circle logo" /></Link>
            <button onClick={toggleSearch} className="text-white focus:outline-none md:hidden">
              <img src={searchIcon} alt="search Icon" />
            </button>
          </div>
          <div className={`relative w-full max-w-lg ${isSearchOpen ? 'block' : 'hidden'} md:block`}>
          ...
          <div className="hidden md:flex flex-col sm:flex-row gap-6 w-4/5 sm:w-3/10 lg:w-1/3 justify-between mx-auto lg:mx-0">
            <Link to="/SignUp" className="text-white hover:text-white border bg-primaryOrange border-primaryOrange px-4 py-2 rounded-xl w-full lg:w-48 md:w-36 text-center font-bold mb-4 sm:mb-0">SIGN UP</Link>
            <Link to="/Login" className="text-primaryOrange hover:text-white border border-primaryOrange px-4 py-2 rounded-xl w-full lg:w-48 md:w-36 text-center font-bold">SIGN IN</Link>
          </div>
        </div>
        <div className={`md:hidden flex-col items-center ${isMenuOpen ? 'flex' : 'hidden'}`}>
          <Link to="/SignUp" className="text-white hover:text-white border bg-primaryOrange border-primaryOrange px-4 py-2 rounded-xl w-full text-center font-bold mb-4">SIGN UP</Link>
          <Link to="/Login" className="text-primaryOrange hover:text-white border border-primaryOrange px-4 py-2 rounded-xl w-full text-center font-bold">SIGN IN</Link>
        </div>
      </nav>
  ...
  <div className="flex flex-col w-full md:w-2/5 gap-5">
    ...
    <div className="flex flex-col md:flex-row mt-4 gap-2 ">
      <div className=' md:hidden w-full'>
        <img src={heroMiniImage} alt="mini version of the hero image w-full" />
      </div>
...
{/* popular */}
      ...
      <div className="mt-12 flex flex-col md:flex-row gap-4 max-h-max border border-grey-200 border-l-0 border-r-0">
          ...
          <h2 className="text-2xl md:text-2xl font-bold text-primaryOrange"><span className='underlines'>Popular O</span>N ChatCircle</h2>
          <p className='text-xl md:text-5xl font-normal leading-tight text-textGrey'>Find answers, share knowledge, and connect with passionate people</p>
          </div>
          <div className=' md:hidden w-full'>
        <img src={popularMini} alt="mini version of the popular threads image w-full" />
      </div>
      </div>
  ...
  <div className="w-full md:w-2/4 flex flex-col gap-6">
    ...
    <p className='text-xl md:text-5xl font-normal leading-tight text-textGrey tracking-wider'>Have a closer look at trending topics and discussions in our forum</p>
  </div>
