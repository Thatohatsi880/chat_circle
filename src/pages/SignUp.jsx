...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.values(newErrors).every((error) => error === '')) {
      try {
        const response = await fetch('https://chat-circle-zib6.onrender.com/auth/register.....signup', {
          method: 'POST',
          headers: {
        ...
        <div className="w-full max-w-xl bg-white p-8 rounded-lg shadow-md">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              ...
              />
              {errors.username && <p className="text-red-500 mt-5">{errors.username}</p>}
            </div>
            <div className="mb-8 relative">
              ...
              {errors.password && <small className="text-red-500 m-0 p-0" >{errors.password}</small>}
            </div>
            <button type="submit" className="w-full h-12 bg-primaryOrange text-white py-2 rounded-xl hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-opacity-50">
              SIGN UP
            </button>
          </form>
